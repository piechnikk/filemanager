    ...
    form.parse(req, function (err, fields, files) {
        if (files.imagetoupload.length > 1) {
            for (let i = 0; i < files.imagetoupload.length; i++) {
                ...
                filesTab.push({ id: id, name: files.imagetoupload[i].name, path: files.imagetoupload[i].path, size: files.imagetoupload[i].size, type: files.imagetoupload[i].type, savedate: new Date().getTime(), download: path, extension: files.imagetoupload[i].path.split(".")[1] })
                id++
            }
        } else {
            var path = ""
            ...
            filesTab.push({ id: id, name: files.imagetoupload.name, path: files.imagetoupload.path, size: files.imagetoupload.size, type: files.imagetoupload.type, savedate: new Date().getTime(), download: path, extension: files.imagetoupload.path.split(".")[1] })
            id++
        }
